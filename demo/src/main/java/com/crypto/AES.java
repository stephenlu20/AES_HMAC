...
public class AES {
    ...
    private byte[][] byteArrToState (byte[] byteArr) {
    ...
    private byte[] stateToByteArr (byte[][] state) {
    ...
    public byte[] encryptBlock(byte[] input) {
        ...
        byte[][] state = byteArrToState(input);
    ...
    // Encrypt longer plain text, but has to be multiple of 16 bytes
    ...
    // this section begins all the methods to decrypt
    // an aes cipher block
    // all the methods are very similar, but are either inverse methods or use inverse constants, like the s_box

    private void invSubBytes(byte[][] state) {
        for (int i = 0; i < 4; i++) {
            for (int j = 0; j < 4; j++) {
                int sub = state[i][j] & 0xFF;
                state[i][j] = (byte) INV_S_BOX[sub];
            }
        }
    }
    ...
    private void invMixColumns(byte[][] state) {
        for (int c = 0; c < 4; c++) {
            ...
            state[0][c] = (byte) (galoisField(s0, (byte)0x0e) ^ galoisField(s1, (byte)0x0b) ^ galoisField(s2, (byte)0x0d) ^ galoisField(s3, (byte)0x09));
            state[1][c] = (byte) (galoisField(s0, (byte)0x09) ^ galoisField(s1, (byte)0x0e) ^ galoisField(s2, (byte)0x0b) ^ galoisField(s3, (byte)0x0d));
            state[2][c] = (byte) (galoisField(s0, (byte)0x0d) ^ galoisField(s1, (byte)0x09) ^ galoisField(s2, (byte)0x0e) ^ galoisField(s3, (byte)0x0b));
            state[3][c] = (byte) (galoisField(s0, (byte)0x0b) ^ galoisField(s1, (byte)0x0d) ^ galoisField(s2, (byte)0x09) ^ galoisField(s3, (byte)0x0e));
        }
    }

    public byte[] decryptBlock(byte[] input) {
        if (input.length != 16) {
            throw new IllegalArgumentException("Expecting 16 bytes. Input is " + input.length + " bytes long");
        }
    ...
    public byte[] decryptBlocks(byte[] input) {
        ...
        for (int i = 0; i < input.length; i += 16) {
            ...
            for (int j = 0; j < 16; j++) {
                plaintext[i + j] = decryptedBlock[j];
            }
        }
        return plaintext;
    }
